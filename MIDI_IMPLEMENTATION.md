...
- **Mode Selection:** Currently uses PC3 pin at startup for mode selection (MIDI/Programmer).
- **Current MIDI Features:** The implementation includes TIM2 for buzzer functionality and PC2-PC5 GPIOs for keyboard input scanning (generating MIDI Note On/Off messages).
- **Future Considerations:** These specific features (PC3 mode switch, TIM2 buzzer, PC2-PC5 keyboard input) are tied to the current implementation and are **planned for removal or significant redesign** in future revisions, focusing solely on UART-to-USB MIDI conversion.
