...
static const uint8_t programmer_special_hid_desc[] = {
    ...
    HID_REPORT_ID(0xac) HID_USAGE(0x01),
    HID_FEATURE(HID_DATA | HID_VARIABLE | HID_ABSOLUTE),
    HID_REPORT_COUNT(78),
    HID_REPORT_ID(0xad) HID_USAGE(0x01),
    HID_FEATURE(HID_DATA | HID_VARIABLE | HID_ABSOLUTE),
    HID_REPORT_COUNT_N(263, 2),
    HID_REPORT_ID(0xae) HID_USAGE(0x01),
    HID_FEATURE(HID_DATA | HID_VARIABLE | HID_ABSOLUTE),
    HID_COLLECTION_END,
};
...
static const uint8_t programmer_config_descriptor[] = {
    ...
    sizeof(programmer_special_hid_desc),
    0x00,
...
const static struct usb_string_descriptor_struct string0
    __attribute__((section(".rodata"))) = {4, 3, {0x0409}}; // Language ID

// Separate string descriptors for Manufacturer and Product for Programmer mode
const static struct usb_string_descriptor_struct string1_prog
    __attribute__((section(".rodata"))) = {
        sizeof(STR_MANUFACTURER_PROG), 3,
        STR_MANUFACTURER_PROG}; // Programmer Manufacturer String
const static struct usb_string_descriptor_struct string2_prog __attribute__((
    section(".rodata"))) = {sizeof(STR_PRODUCT_PROG), 3,
                            STR_PRODUCT_PROG}; // Programmer Product String

// Separate string descriptors for Manufacturer and Product for MIDI mode
const static struct usb_string_descriptor_struct string1_midi __attribute__((
    section(".rodata"))) = {sizeof(STR_MANUFACTURER_MIDI), 3,
                            STR_MANUFACTURER_MIDI}; // MIDI Manufacturer String
const static struct usb_string_descriptor_struct string2_midi
    __attribute__((section(".rodata"))) = {
        sizeof(STR_PRODUCT_MIDI), 3, STR_PRODUCT_MIDI}; // MIDI Product String

const static struct usb_string_descriptor_struct string3
    __attribute__((section(".rodata"))) = {sizeof(STR_SERIAL), 3,
                                           STR_SERIAL}; // Serial String

// Descriptor Lists
// Programmer Descriptors
const static struct descriptor_list_struct descriptor_list_programmer[] = {
    {0x00000100, programmer_device_descriptor,
     sizeof(programmer_device_descriptor)},
    {0x00000200, programmer_config_descriptor,
     sizeof(programmer_config_descriptor)},
    {0x00002200, programmer_special_hid_desc,
     sizeof(programmer_special_hid_desc)},
    {0x00002100, programmer_config_descriptor + 18, 9},
    {0x00000300, (const uint8_t *)&string0, 4}, // Language ID
    {0x04090301, (const uint8_t *)&string1_prog,
     sizeof(STR_MANUFACTURER_PROG)}, // Programmer Manufacturer String (Index 1
                                     // in device descriptor)
    {0x04090302, (const uint8_t *)&string2_prog,
     sizeof(STR_PRODUCT_PROG)}, // Programmer Product String (Index 2 in device
                                // descriptor)
    {0x04090303, (const uint8_t *)&string3,
     sizeof(STR_SERIAL)}}; // Serial String (Index 3 in device descriptor)
...
const static struct descriptor_list_struct descriptor_list_midi[] = {
    ...
    {0x00000300, (const uint8_t *)&string0, 4}, // Language ID
    {0x04090301, (const uint8_t *)&string1_midi,
     sizeof(STR_MANUFACTURER_MIDI)}, // MIDI Manufacturer String (Index 1 in
                                     // device descriptor)
    {0x04090302, (const uint8_t *)&string2_midi,
     sizeof(STR_PRODUCT_MIDI)}, // MIDI Product String (Index 2 in device
                                // descriptor)
    {0x04090303, (const uint8_t *)&string3,
     sizeof(STR_SERIAL)}}; // Serial String (Index 3 in device descriptor)
...
void set_usb_mode_programmer() {
  ...
  active_descriptor_list_entries = sizeof(descriptor_list_programmer) /
                                   sizeof(struct descriptor_list_struct);
}

void set_usb_mode_midi() {
  active_descriptor_list = descriptor_list_midi;
  active_descriptor_list_entries =
      sizeof(descriptor_list_midi) / sizeof(struct descriptor_list_struct);
}
